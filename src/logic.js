...
function autoSave() {
    ...
    // 🚨 修正：不再強制清空 enemy 或 inBattle
    // 我們要「如實」儲存當前狀態，這樣 F5 才會停留在原地
    const data = { 
        player, 
        inventory, 
        gameState: { 
            ...gameState, 
            // 只有 isProcessingTurn 要強制 false，避免讀檔後按鈕全灰
            isProcessingTurn: false,
            // logs 也要存，不然 F5 後上面的戰鬥紀錄會消失
            logs: gameState.logs 
        }, 
        stats 
    };
...
export function checkSaveAndStart() {
    ...
    try {
        ...
        // 🚨 關鍵修正：讀檔後的狀態還原
        gameState.isProcessingTurn = false; // 解鎖按鈕
        gameState.canAct = true;            // 允許互動
        gameState.cooldownTimerId = null;   // 清除舊計時器

        // 根據讀取到的模式，決定要不要恢復戰鬥或商人
        if (gameState.mode === 'battle' && gameState.enemy) {
            gameState.inBattle = true;
            // 不用做什麼，因為 UI.renderMainScreen 會根據 mode='battle' 畫出怪物
        ...
        // 這裡不寫 Log 了，不然每次 F5 都會洗版
        // UI.addLog(`歡迎回來...`, "log-system"); 
...
function startCooldown(seconds) {
    ...
    gameState.cooldownTimerId = setInterval(() => {
        ...
        if (t <= 0) { 
            ...
            // 只有在正常探索模式才刷新回主畫面，避免干擾戰鬥或商人
            if (gameState.mode === 'normal') UI.renderMainScreen(); 
        } else { 
...
export function useItem(i) { const item = inventory[i]; if (item.type === "equip") { equipItem(i); return; } let used = false; if (item.type === "heal") { player.hp = Math.min(player.maxHP, player.hp + item.value); UI.addLog(`🧪 使用 ${item.name}，恢復 ${item.value} HP！`); used = true; } else if (item.type === "food") { player.hunger = Math.min(player.hungerMax, player.hunger + item.value); UI.addLog(`🍱 吃掉 ${item.name}，恢復 ${item.value} 飢餓！`); used = true; } else if (item.type === "mana") { player.mp = Math.min(player.maxMP, player.mp + item.value); UI.addLog(`🧪 使用 ${item.name}，恢復 ${item.value} MP！`); used = true; } if (used && item.type !== "equip") { if(item.count > 1) item.count--; else inventory.splice(i, 1); UI.updateStatus(); UI.updateInventory(); autoSave(); } }
...
function startBossBattle(bossId) {
    ...
    UI.addLog(`⚠️ 警告：${gameState.enemy.name} 出現了！`, "log-critical");
    // 不用手動 renderMainScreen，addLog 會做
}
...
function startBattle(zone, difficulty = 1) {
    ...
    gameState.inBattle = true;
    gameState.isProcessingTurn = false;
    gameState.battleLog = []; // 清空 Log 其實這裡可以用 gameState.logs=[] 但我們用统一 Log 所以不用清
...
export function handleCombat(action, skillId = null) {
    ...
    if (playerGoesFirst) {
        ...
        if (gameState.enemy && gameState.enemy.hp > 0) {
            setTimeout(() => { doEnemyMove(); gameState.isProcessingTurn = false; UI.renderMainScreen(); }, 600);
        } else { 
        ...
        doEnemyMove();
        if (player.hp > 0) {
            setTimeout(() => { doPlayerMove(action, skillId); gameState.isProcessingTurn = false; UI.renderMainScreen(); }, 600);
        } else { 
            // 玩家死
        }
    }
...
export function runAway() {
    ...
    if (Math.random() * 100 < escapeChance) {
        gameState.inBattle = false; gameState.mode = "normal"; gameState.enemy = null;
        UI.addLog("🏃‍♂️ 你成功脫離了戰鬥。", "log-battle");
        gameState.isProcessingTurn = false; autoSave(); UI.renderMainScreen(); UI.showMainActions();
    } else {
        UI.addLog("🚫 逃跑失敗！被敵人抓住了！", "log-critical");
        setTimeout(() => { doEnemyMove(); gameState.isProcessingTurn = false; UI.renderMainScreen(); }, 500);
    }
}
