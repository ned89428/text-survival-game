...
function autoSave() {
    ...
    const data = { player, inventory, gameState: { ...gameState, inBattle: false, enemy: null, mode: "normal", logs: [] }, stats };
    localStorage.setItem('LegendSurvival_Save', JSON.stringify(data));
}

// src/logic.js 的 checkSaveAndStart 函式
...
export function checkSaveAndStart() {
    ...
    try {
        ...
        // ================================================
        // 🚨 BUG 修復：強制重置行動狀態
        // 避免讀檔後因為卡在冷卻時間而導致按鈕消失
        // ================================================
        gameState.inBattle = false;       // 確保不在戰鬥
        gameState.canAct = true;          // 強制可以行動
        gameState.cooldownTimerId = null; // 清空計時器 ID
        gameState.merchantActive = false; // 讀檔時先關閉商人，避免介面錯亂
        gameState.mode = "normal";        // 回到正常模式
        gameState.enemy = null;           // 清空敵人
        // ================================================
        ...
        // 這裡非常重要：因為上面強制 canAct = true 了，所以這裡呼叫 showMainActions 一定會顯示按鈕
        UI.showMainActions(); 
...
function startCooldown(seconds) {
    if (seconds <= 0) { gameState.canAct = true; UI.renderMainScreen(); return; } // 結束後刷新畫面
    ...
    gameState.cooldownTimerId = setInterval(() => {
        ...
        if (t <= 0) {
            ...
            if (!gameState.merchantActive) UI.renderMainScreen(); // 結束後刷新
        } else {
            UI.updateCooldownButtons(t);
        }
    }, 1000);
...
function canDoExplore(costHunger, costEnergy) {
    ...
    if (player.hunger <= 0) {
        ...
        failed = true;
    } else { player.hunger = Math.max(0, player.hunger - costHunger); }

    if (player.energy <= 0) {
        player.hp -= 10;
        UI.addLog("⚠️ 累得要死還強行探索... HP -10", "log-critical");
        failed = true;
    } else { player.energy = Math.max(0, player.energy - costEnergy); }
    ...
    if (player.state === "中毒") {
        player.hp -= 5;
        UI.addLog("中毒發作，你損失 5 HP。", "log-battle");
...
export function exploreNearby() {
    if (!canDoExplore(5, 5)) return;
    stats.exploredNearby++;
    UI.addLog("🔍 你在附近小心搜索……");
    startCooldown(COOLDOWNS.NEARBY);
    setTimeout(() => { if (player.alive) { resolveNearby(); advanceDay(); } }, COOLDOWNS.NEARBY * 1000);
}

export function exploreDungeon() {
    if (!canDoExplore(10, 15)) return;
    stats.exploredDungeon++;
    UI.addLog("🕳️ 你走入陰暗的地下城……");
    startCooldown(COOLDOWNS.DUNGEON);
    setTimeout(() => { if (player.alive) { resolveDungeon(); advanceDay(); } }, COOLDOWNS.DUNGEON * 1000);
}

export function exploreExpedition() {
    if (!canDoExplore(25, 30)) return;
    stats.exploredExpedition++;
    UI.addLog("🏕️ 你踏上長時間遠征……");
    startCooldown(COOLDOWNS.EXPEDITION);
    setTimeout(() => { if (player.alive) { resolveExpedition(); advanceDay(); } }, COOLDOWNS.EXPEDITION * 1000);
}
...
function lootRandomItem(type) {
    if (type === "food") {
        const item = { ...CONSUMABLES[0] }; item.sellPrice = 5; inventory.push(item);
        ...
        if (Math.random() < 0.5) {
            ...
            const mat = { name: "普通素材", emoji: "🧩", type: "material", usable: false, sellPrice: 10 };
            inventory.push(mat);
            UI.addLog("🧩 你撿到一些普通素材。", "log-system");
        }
    }
    else if (type === "material") {
        const mat = { name: "普通素材", emoji: "🧩", type: "material", usable: false, sellPrice: 5 };
        inventory.push(mat);
        UI.addLog("🧩 你撿到一些普通素材。", "log-system");
    }
    UI.updateInventory();
...
export function equipItem(index) {
    ...
    if (currentEquip) inventory.push(currentEquip);
    player.equipment[slot] = item;
    inventory.splice(index, 1);
    UI.addLog(`你裝備了 ${item.name}。`); // 使用 addLog 而不是 writeEvent
    UI.updateStatus();
    UI.updateInventory();
    autoSave();
}
...
export function dropItem(i) {
    UI.addLog(`丟棄 ${inventory[i].name}`, "log-system");
    inventory.splice(i, 1);
    UI.updateInventory();
    autoSave();
}
...
function startBattle(zone, difficulty = 1) {
    ...
    if (candidates.length === 0) {
        candidates = ENEMIES.filter(e => e.zone === zone);
    ...
    // UI.renderMainScreen() 會在 addLog 裡自動被呼叫，所以不用手動寫
}
...
export function sellAllMaterials() {
    ...
    if (total > 0) {
        player.gold += total;
        UI.addLog(`出售素材獲得 ${total} G`, "log-system");
        UI.updateStatus();
        UI.updateInventory();
        autoSave();
    } else { UI.addLog("沒有素材可賣"); }
}

export function closeMerchant() {
    gameState.merchantActive = false;
    gameState.merchantGoods = [];
    gameState.mode = "normal"; // 切回正常
    UI.addLog("結束交易");
    UI.updateInventory();
}
...
function gameOver(reason) {
    ...
    deathSummary.innerHTML = `
        <p>存活天數：${player.day} | 原因：${reason}</p>
        <p>等級：Lv ${player.level} | 擊殺數：${stats.kills}</p>
        <p>背包：${bagList}</p>
    `;
    document.getElementById("overlay").style.display = "flex";
...
export function chooseJob(jobKey) {
    ...
    if (jobKey === "warrior") { player.job = "戰士"; player.str = 5; player.con = 5; } 
    else if (jobKey === "archer") { player.job = "弓箭手"; player.agi = 6; player.str = 2; player.int = 2; } 
    else if (jobKey === "rogue") { player.job = "盜賊"; player.str = 3; player.agi = 5; player.int = 2; } 
    else if (jobKey === "mage") { player.job = "法師"; player.int = 8; player.con = 1; player.agi = 1; }
