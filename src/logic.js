...
function autoSave() {
    ...
    const data = { player, inventory, gameState: { ...gameState, inBattle: false, enemy: null, battleLog: [], logs: gameState.logs }, stats };
...
function canDoExplore(costHunger, costEnergy) {
    ...
    // if (gameState.logs.length > 0) gameState.logs = []; 
...
function lootRandomItem(type) { if (type === "food") { const item = { ...CONSUMABLES[0] }; item.sellPrice = 5; inventory.push(item); UI.addLog(`🍱 你找到 ${item.name}，已放入背包。`, "log-system"); } else if (type === "treasure") { if (Math.random() < 0.5) { const potion = { ...CONSUMABLES[Math.floor(Math.random() * CONSUMABLES.length)] }; if(potion.type === "food") potion.type = "heal"; potion.sellPrice = Math.floor(potion.price / 2); inventory.push(potion); UI.addLog(`✨ 你找到 ${potion.name}！`, "log-system"); } else { const mat = { name: "普通素材", emoji: "🧩", type: "material", usable: false, sellPrice: 10 }; inventory.push(mat); UI.addLog("🧩 你撿到一些普通素材。", "log-system"); } } else if (type === "material") { const mat = { name: "普通素材", emoji: "🧩", type: "material", usable: false, sellPrice: 5 }; inventory.push(mat); UI.addLog("🧩 你撿到一些普通素材。", "log-system"); } UI.updateInventory(); autoSave(); }
export function equipItem(index) { const item = inventory[index]; if (!item || item.type !== "equip") return; const slot = item.slot; const currentEquip = player.equipment[slot]; if (currentEquip) inventory.push(currentEquip); player.equipment[slot] = item; inventory.splice(index, 1); UI.addLog(`你裝備了 ${item.name}。`); UI.updateStatus(); UI.updateInventory(); autoSave(); }
export function unequipItem(slot) { const item = player.equipment[slot]; if (!item) return; player.equipment[slot] = null; inventory.push(item); UI.addLog(`你卸下了 ${item.name}。`); UI.updateStatus(); UI.updateInventory(); autoSave(); }
export function useItem(i) { const item = inventory[i]; if (item.type === "equip") { equipItem(i); return; } if (item.type === "heal") { player.hp = Math.min(player.maxHP, player.hp + item.value); UI.addLog(`🧪 使用 ${item.name}，恢復 ${item.value} HP！`); } else if (item.type === "food") { player.hunger = Math.min(player.hungerMax, player.hunger + item.value); UI.addLog(`🍱 吃掉 ${item.name}，恢復 ${item.value} 飢餓！`); } else if (item.type === "mana") { player.mp = Math.min(player.maxMP, player.mp + item.value); UI.addLog(`🧪 使用 ${item.name}，恢復 ${item.value} MP！`); } if (item.type !== "equip") { inventory.splice(i, 1); UI.updateStatus(); UI.updateInventory(); autoSave(); } }
export function sellOneItem(i) { if (!gameState.merchantActive) { UI.addLog("需在商人處才能出售。"); return; } const item = inventory[i]; if (!item.sellPrice) return; player.gold += item.sellPrice; UI.addLog(`賣出 ${item.name} (+${item.sellPrice} G)`, "log-system"); inventory.splice(i, 1); UI.updateStatus(); UI.updateInventory(); renderMerchantUI(`你賣掉了 ${item.name} (+${item.sellPrice} G)`); autoSave(); }
...
export function handleCombat(action, skillId = null) {
    ...
    if (playerGoesFirst) {
        ...
        if (gameState.enemy && gameState.enemy.hp > 0) {
            setTimeout(() => { doEnemyMove(); gameState.isProcessingTurn = false; }, 600);
        } else { gameState.isProcessingTurn = false; }
    } else {
        UI.addLog(`🐢 敵人動作較快！`, "log-system");
        doEnemyMove();
        if (player.hp > 0) {
            setTimeout(() => { doPlayerMove(action, skillId); gameState.isProcessingTurn = false; }, 600);
        } else { gameState.isProcessingTurn = false; }
    }
}
...
export function runAway() {
    ...
    if (Math.random() * 100 < escapeChance) {
        ...
        gameState.isProcessingTurn = false; autoSave();
    } else {
        UI.addLog("🚫 逃跑失敗！被敵人抓住了！", "log-critical");
        setTimeout(() => { doEnemyMove(); gameState.isProcessingTurn = false; }, 500);
    }
}
...
function winBattle() {
    ...
    if (Math.random() < dropChance) {
        if (Math.random() < 0.8) {
            const newItem = generateRandomEquipment(enemy.lvl); inventory.push(newItem); UI.addLog(`🎁 掉落裝備：${newItem.name}`, "log-system");
...
export function buyItem(index) {
    ...
    player.gold -= item.price; inventory.push({ ...item });
...
export function closeMerchant() {
    ...
    UI.addLog("結束交易"); UI.updateInventory();
}
