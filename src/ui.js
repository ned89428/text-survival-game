...
export function renderMainScreen() {
    ...
    if (gameState.logs && gameState.logs.length > 0) {
        // 使用 slice().reverse() 讓最新的顯示在最上面? 
        // 還是維持原樣(最新的在最下面)? 
        // 根據你之前的需求，最新的在最下面(配合 CSS 的 overflow-y: auto)
    ...
    if (gameState.mode === "battle") {
        ...
        // 關鍵修復：確保切換回正常模式時，下方按鈕會顯示出來
        document.getElementById("actions").style.display = "block";
        // 這裡順便呼叫一次 showMainActions 確保按鈕內容存在
        // 但為了避免遞迴或閃爍，我們依靠外部邏輯呼叫，或者在這裡檢查內容
        if (document.getElementById("actions").innerHTML.trim() === "") {
             showMainActions();
        }
    }

    // 3. 組合 HTML (只更新 eventBox，actions 由另外的函式控制)
    box.innerHTML = `
        ${topSection}
        ${logSection}
        ${actionButtons}
...
export function updateInventory() {
    ...
    box.innerHTML = `
        <div class="inv-title">🎒 背包（${inventory.length}）</div>
        ${equipHtml}
...
export function showMainActions() {
    ...
    // 這裡必須重新生成 HTML，否則 actions 區塊會是空的！
